...
bool app::net::Server::processLobbyReady(int id)
{
	...
	auto const predicate =
		[&](app::net::Lobby const & lobby) { return lobbyId == lobby.getId(); };
	if (auto const & result = std::find_if(m_lobbies.begin(), m_lobbies.end(), predicate); result != m_lobbies.end())
	{
		auto const & lobby = *result;
